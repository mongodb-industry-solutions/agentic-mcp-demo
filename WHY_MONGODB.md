...
**Concurrency primitive: atomic claims via `find_one_and_update`.** Memory extraction can be queued from two paths after a kill+restart — the boot-time backlog scan AND the change-stream replay both see the same closure event. Coordination uses `find_one_and_update` with a `memories_extraction_started_at` field and a 5-minute stale-claim TTL: only one task wins the transition into a claimed state, a crashed claim becomes reclaimable after the TTL, no Redis or Zookeeper involved. The pattern generalises — any "exactly-once per state transition" task in the framework uses the same Atlas-native primitive.
