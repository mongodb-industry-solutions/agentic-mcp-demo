...
- `web/shell.py` + `web/shell.html` — Browser-based equivalent of `main.py` (FastAPI + WebSocket, `localhost:8070`). Includes a **Reset demo data** button that re-runs both seed scripts' `--reset` pipeline from the browser via `web/seed_runner.py`, streaming progress live. Phase A of `MULTI_SESSION_PLAN.md` (single-user-safe; per-session isolation is Phase B)
