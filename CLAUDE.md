...
- `agents/domain_agent.py` + `agents/catalog/` — `DomainAgent` (Phase 1): domain-scoped specialists (`ibn_agent`, `dtw_agent`) with their own system prompts and ReAct loops over only their domain's MCP tools. Gated by the `AGENT_MODE` env flag (unset → legacy routing; `1` → IBN agent; `all` → every catalog agent; or `ibn,dtw` list). Agent cards are synced to the vector-indexed `agent_registry.agent_cards` collection at startup
