...
- `agents/domain_agent.py` + `agents/catalog/` — `DomainAgent` (Phases 1–2): domain-scoped specialists (`ibn_agent`, `dtw_agent`) with their own system prompts and ReAct loops over only their domain's MCP tools; each agent runs its own in-domain Stage 2 server selection. Gated by the `AGENT_MODE` env flag (unset → legacy routing; `1` → IBN agent; `all` → every catalog agent; or `ibn,dtw` list). Agent cards are synced to the vector-indexed `agent_registry.agent_cards` collection at startup; when a turn spans two or more agent domains, the shell ranks agents by `$vectorSearch` over the cards, splits the request into per-agent sub-tasks (gpt-4o-mini), runs the agents concurrently, and synthesizes one answer (gpt-4o)
