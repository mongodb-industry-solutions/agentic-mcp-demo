...
**Behind a reverse proxy (optional):** the three apps support being fronted by one host under sub-paths — shell at `/`, IBN dashboard at `/ibn/`, DTW dashboard at `/dtw/` — with WebSocket upgrade + long read timeouts. Set `DEMO_BIND_HOST=127.0.0.1` so the uvicorn ports are reachable only through the proxy. The client code auto-detects the sub-path mount (WS uses `wss://` on TLS, and each dashboard derives its prefix from the URL), so no app config is needed — just proxy `/`, `/ibn/`, `/dtw/` to ports 8070/8060/8080 (strip the `/ibn`/`/dtw` prefix) and enable WebSocket upgrades.
