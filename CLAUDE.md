...
- `web/shell.py` + `web/shell.html` — Browser-based equivalent of `main.py` (FastAPI + WebSocket, `localhost:8070`). Per-browser-session isolation (`MULTI_SESSION_PLAN.md`): each session gets its own `OrchestratorAgent` with a `s_<token>_` collection prefix (namespacing the mutable demo collections + workstreams/memories; reference data + vector indexes shared), capped by `DEMO_MAX_SESSIONS` and idle-reaped via `DEMO_SESSION_TTL_SEC`. Includes a per-session **Reset demo data** button (`web/seed_runner.py`, streams progress live). Sessions persist via a `localStorage` token + `init` handshake. MCP servers read `DEMO_PREFIX` from their env (empty → shared/default lane, unchanged for the CLI)
